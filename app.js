(()=>{var L=window.DROPIC_BASE||"",T=window.DROPIC_API||"",Z=window.matchMedia("(max-width: 768px)").matches,I=null,N=null,B=null,l=document.getElementById("app"),U=document.getElementById("userbox"),O=document.getElementById("toast"),m=null,w={dark_mode:1,notif_likes:1,notif_comments:1,notif_follows:1,notif_mentions:1,notif_messages:1};function r(e){return L+e}function E(e){return e?e.startsWith("http")?e:T+e:""}function i(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function p(e){O.textContent=e,O.classList.add("show"),clearTimeout(p._t),p._t=window.setTimeout(()=>O.classList.remove("show"),2200)}async function d(e,t={}){let a={"Content-Type":"application/json",...t.headers},s=localStorage.getItem("token");s&&(a.Authorization=`Bearer ${s}`);let o=await fetch(T+e,{...t,headers:a,credentials:"include"}),n=await o.json().catch(()=>({}));if(!o.ok)throw new Error(n.error||"Something went wrong");return n}async function A(e,t){let a=localStorage.getItem("token"),s={};a&&(s.Authorization=`Bearer ${a}`);let o=await fetch(T+e,{method:"POST",body:t,headers:s,credentials:"include"}),n=await o.json().catch(()=>({}));if(!o.ok){let u=new Error(n.error||"Upload failed");throw u.status=o.status,u.body=n,u}return n}function H(e){let t=new Date(e.replace(" ","T")+"Z"),a=Math.floor((Date.now()-t.getTime())/1e3);return a<60?"just now":a<3600?`${Math.floor(a/60)}m`:a<86400?`${Math.floor(a/3600)}h`:a<604800?`${Math.floor(a/86400)}d`:t.toLocaleDateString()}function x(e){return i(e).replace(/(^|\s)#([a-zA-Z0-9_]+)/g,(t,a,s)=>{let o=r(`/hashtag/${s}`);return`${a}<a href="${o}" data-nav="${o}" class="taglink">#${s}</a>`}).replace(/(^|\s)@([a-zA-Z0-9_]+)/g,(t,a,s)=>{let o=r(`/${s}`);return`${a}<a href="${o}" data-nav="${o}" class="taglink">@${s}</a>`})}function h(e,t=34){let a=i((e.display_name||"?").charAt(0).toUpperCase()),s="is_verified"in e&&e.is_verified?'<span class="verify">\u2713</span>':"";return e.avatar?`<span class="avwrap" style="width:${t}px;height:${t}px">${s}<img class="avatar" src="${E(e.avatar)}" alt="" /></span>`:`<span class="avwrap" style="width:${t}px;height:${t}px">${s}<span class="avatar">${a}</span></span>`}var M={heart:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 0 0 0-7.8Z"/></svg>',bubble:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 11.5a8.5 8.5 0 0 1-12.4 7.5L3 21l2-5.6A8.5 8.5 0 1 1 21 11.5Z"/></svg>',repost:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 2l4 4-4 4"/><path d="M3 11v-1a4 4 0 0 1 4-4h14"/><path d="M7 22l-4-4 4-4"/><path d="M21 13v1a4 4 0 0 1-4 4H3"/></svg>',bookmark:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2Z"/></svg>',pin:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 17v5"/><path d="M9 10.8V5l-1.5-2h9L15 5v5.8L18 16H6Z"/></svg>',share:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="m8.6 13.5 6.8 4M15.4 6.5l-6.8 4"/></svg>',dots:'<svg viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="19" cy="12" r="2"/></svg>',bell:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.7 21a2 2 0 0 1-3.4 0"/></svg>'};function k(){if(!m){U.innerHTML=`<a href="${r("/login")}" class="btn" data-nav="${r("/login")}">Log in</a>`;return}U.innerHTML=`
  ...
  </div>`}function V(){let e=l.querySelector("textarea[name=body]"),t=l.querySelector("#ccount");if(e&&t){let c=()=>{t.textContent=`${e.value.length}/280`,t.classList.toggle("full",e.value.length>=280)};e.addEventListener("input",c),c()}let a=l.querySelector("#pollbox"),s=l.querySelector("#pollbtn");s&&s.addEventListener("click",()=>a.hidden=!a.hidden);let o=l.querySelector("#draftbtn");o&&o.addEventListener("click",async()=>{let c=e.value.trim();if(!c)return p("Nothing to save");try{await d("/api/misc/drafts",{method:"POST",body:JSON.stringify({body:c})}),p("Saved to drafts")}catch(g){p(g.message)}});let n=l.querySelector('input[name="image"]');n&&n.addEventListener("change",()=>{let c=n.files?.[0];document.getElementById("picname").textContent=c?c.name:""});let u=document.getElementById("postform");u.addEventListener("submit",async c=>{c.preventDefault();let g=new FormData(u);if(!String(g.get("body")??"").trim()&&!g.get("image"))return p("Add text or a photo");let _=String(g.get("poll_question")??"").trim();if(_){let b=[String(g.get("poll_op1")??""),String(g.get("poll_op2")??"")].filter(Boolean);if(b.length<2)return p("Polls need 2+ options");g.append("poll_question",JSON.stringify({question:_,options:b})),g.delete("poll_op1"),g.delete("poll_op2")}try{await A("/api/posts",g),u.reset(),document.getElementById("picname").textContent="",t&&(t.textContent="0/280"),p("Posted"),v()}catch(b){let $=b;if($.status===409&&$.body?.original_username){let C=$.body.original_display_name||$.body.original_username,z=$.body.exact_match?"exact match":`match score ${$.body.distance}/64`;p(`Already uploaded by @${$.body.original_username} (${z})`)}else p($.message)}})}async function X(){try{let e=await d("/api/stories"),t=new Map;for(let s of e.stories)t.has(s.user_id)||t.set(s.user_id,[]),t.get(s.user_id).push(s);let a=[...t.entries()];return`
    <div class="stories">
  ...
  <article class="post" data-postid="${e.id}">
    ...
    <div class="post-header">
      <a href="${r(`/${i(e.username)}`)}" data-nav="${r(`/${i(e.username)}`)}">${h(e)}</a>
  ...
  <div class="comment" data-comment="${e.id}">
    ...
    </div>`).join("")}</div>`:""}function f(){document.title="Dropicgram",l.innerHTML=`
  <div class="auth-wrap">
  ...
  </div>`;let e=Array.from(l.querySelectorAll(".tab"));e.forEach(t=>t.addEventListener("click",()=>{e.forEach(s=>s.classList.remove("active")),t.classList.add("active");let a=t.dataset.tab;l.querySelector('[data-form="login"]').hidden=a!=="login",l.querySelector('[data-form="register"]').hidden=a!=="register"})),l.querySelectorAll("form[data-form]").forEach(t=>{t.addEventListener("submit",async a=>{a.preventDefault();let s=t.querySelector("button");s.disabled=!0;let o=t.dataset.form==="login"?"login":"register",n=new FormData(t),u={username:String(n.get("username")??""),password:String(n.get("password")??"")};o==="register"&&(u.display_name=String(n.get("display_name")||u.username));try{let c=await d(`/api/auth/${o}`,{method:"POST",body:JSON.stringify(u)});c.token&&localStorage.setItem("token",c.token),m=c.user,k(),R(),history.pushState({},"",r("/")),v()}catch(c){p(c.message),s.disabled=!1}})})}async function et(){if(!m)return f();document.title="Home \xB7 Dropicgram",l.innerHTML=`${Y()}<div id="stories"></div><div id="feed"><div class="skeleton"></div><div class="skeleton"></div></div><div class="load-more"><button class="btn secondary" id="loadmore" hidden>Load more</button></div>`,V();let e=await X();document.getElementById("stories").innerHTML=e,bt();let t=null,a=!1;async function s(n=!1){if(!a){a=!0;try{let u=new URLSearchParams;t&&!n&&u.set("before",t);let c=document.getElementById("mediafilter");c&&c.classList.contains("active")&&u.set("media","1");let g=await d(`/api/posts/feed?${u.toString()}`),y=document.getElementById("feed");if(n&&(y.innerHTML=""),!g.posts.length){n&&(y.innerHTML=`<div class="empty">Nothing here yet.<br />Follow some people or post your first pic!<br /><a class="btn" data-nav="${r("/explore")}" href="${r("/explore")}">Explore</a></div>`);return}y.innerHTML+=g.posts.map(q).join(""),t=g.posts[g.posts.length-1].created_at;let b=document.getElementById("loadmore");b&&(b.hidden=g.posts.length<25),P(y),S()}catch(u){p(u.message)}finally{a=!1}}}let o=document.getElementById("loadmore");if(o&&o.addEventListener("click",()=>s()),Z){let n=0;l.addEventListener("touchstart",u=>{window.scrollY<=0?n=u.touches[0].clientY:n=0},{passive:!0}),l.addEventListener("touchend",u=>{n&&window.scrollY<=0&&u.changedTouches[0].clientY>n+80&&s(!0),n=0},{passive:!0})}s(!0)}async function at(){document.title="Explore \xB7 Dropicgram",l.innerHTML=`
    ...
    <div class="load-more"><button class="btn secondary" id="loadmore" hidden>Load more</button></div>`;let e=document.getElementById("globalsearch");e.addEventListener("keydown",a=>{if(a.key==="Enter"){let s=e.value.trim();s&&(history.pushState({},"",r(`/search/${encodeURIComponent(s)}`)),v())}});try{let a=await d("/api/search/trending");document.querySelector(".trend-list").innerHTML=a.trends.length?a.trends.map(s=>`<a class="trend" data-nav="${r(`/hashtag/${i(s.tag)}`)}" href="${r(`/hashtag/${i(s.tag)}`)}">#${i(s.tag)} <span>${s.count}</span></a>`).join(""):'<div class="empty">No trends yet</div>'}catch{}let t=await d("/api/posts/explore");document.getElementById("feed").innerHTML=t.posts.map(q).join("")||'<div class="empty">No posts yet \u2014 be the first!</div>',P(l)}async function st(e){document.title="Search \xB7 Dropicgram",l.innerHTML='<h1 style="margin:4px 4px 14px">Search</h1><div id="results"></div>';let t=await d(`/api/search?q=${encodeURIComponent(e)}`),a="";t.users.length&&(a+=`<div class="card"><h2>People</h2>${t.users.map(s=>`
      <div class="userrow">
        <a href="${r(`/${i(s.username)}`)}" data-nav="${r(`/${i(s.username)}`)}">${h(s,40)} <b>${i(s.display_name)}</b> <span class="username">@${i(s.username)}</span></a>
      </div>`).join("")}</div>`),t.hashtags.length&&(a+=`<div class="card"><h2>Hashtags</h2>${t.hashtags.map(s=>`<a class="trend" data-nav="${r(`/hashtag/${i(s.tag)}`)}" href="${r(`/hashtag/${i(s.tag)}`)}">#${i(s.tag)} <span>${s.count}</span></a>`).join("")}</div>`),t.posts.length&&(a+=`<div id="posts">${t.posts.map(q).join("")}</div>`),!t.users.length&&!t.posts.length&&!t.hashtags.length&&(a='<div class="empty">Nothing found for "'+i(e)+'"</div>'),document.getElementById("results").innerHTML=a,P(l)}async function nt(e){document.title=`#${e} \xB7 Dropicgram`,l.innerHTML=`<h1 style="margin:4px 4px 14px">#${i(e)}</h1><div id="feed"></div>`;let t=await d(`/api/search/hashtag/${encodeURIComponent(e)}`);document.getElementById("feed").innerHTML=t.posts.map(q).join("")||'<div class="empty">No posts with this tag yet.</div>',P(l)}async function j(e){if(!m)return f();document.title=`${e} \xB7 Dropicgram`,l.innerHTML='<div class="skeleton"></div>';try{let t=await d(`/api/posts/user/${encodeURIComponent(e)}`),a=t.user,s=m?.username===a.username;l.innerHTML=`
      <div class="profile">
      ...
      </a>`).join(""):'<div class="empty">No messages yet.</div>'}async function ct(e){if(!m)return f();document.title=`${e} \xB7 Dropicgram`,l.innerHTML=`
    <div class="pagehead">
    ...
    <div class="compose-bar msgbar">
    ...
    </div>`,l.querySelector('[data-action="back"]').addEventListener("click",()=>{history.pushState({},"",r("/messages")),v()});let t=document.getElementById("msgs"),a=document.createElement("div");a.id="typing-ind",a.className="mtime typing-ind",t.appendChild(a);async function s(){let c=await d(`/api/messages/${encodeURIComponent(e)}`);t.innerHTML=c.messages.map(g=>`
      <div class="msg ${g.sender}">
        ${g.image?`<img data-lightbox src="${E(g.image)}" />`:""}
        ${g.body?`<div>${x(g.body)}</div>`:""}
        <div class="mtime">${H(g.created_at)}${g.sender==="me"?` ${g.read?"\u2713\u2713":"\u2713"}`:""}</div>
      </div>`).join("")||'<div class="empty">Say hi \u{1F44B}</div>',t.appendChild(a),t.scrollTop=t.scrollHeight}await s(),B=c=>{c.type==="dm"&&String(c.from)!==String(m?.id)?location.pathname.includes(`/messages/${encodeURIComponent(e)}`)&&s():c.type==="typing"&&String(c.from)!==String(m?.id)?(a.textContent="typing\u2026",clearTimeout(a._t),a._t=window.setTimeout(()=>a.textContent="",2e3)):c.type==="read"&&s()};let o=l.querySelector('input[name="body"]'),n=0;o.addEventListener("input",()=>{let c=Date.now();c-n>1500&&(n=c,Et({type:"typing",to:m&&e?e:""}))}),document.getElementById("msgform").addEventListener("submit",async c=>{c.preventDefault();let g=c.target,y=new FormData(g);if(!(!String(y.get("body")??"").trim()&&!y.get("image")))try{await A(`/api/messages/${encodeURIComponent(e)}`,y),g.reset(),await s(),S()}catch(b){p(b.message)}})}async function lt(){if(!m)return f();document.title="Saved \xB7 Dropicgram",l.innerHTML='<h1 style="margin:4px 4px 14px">\u{1F516} Saved posts</h1><div id="feed"><div class="skeleton"></div></div>';let e=await d("/api/misc/bookmarks");document.getElementById("feed").innerHTML=e.posts.map(q).join("")||'<div class="empty">No saved posts yet.</div>',P(l)}async function mt(){if(!m)return f();document.title="Follow requests \xB7 Dropicgram",l.innerHTML='<h1 style="margin:4px 4px 14px">Follow requests</h1><div id="list"></div>';let e=await d("/api/search/requests");document.getElementById("list").innerHTML=e.users.length?e.users.map(t=>`
      <div class="card userrow">
        <a href="${r(`/${i(t.username)}`)}" data-nav="${r(`/${i(t.username)}`)}">${h(t,40)} <b>${i(t.display_name)}</b></a>
      ...
      </div>`).join(""):'<div class="empty">No drafts saved.</div>',document.querySelectorAll("[data-action=use-draft]").forEach(t=>t.addEventListener("click",()=>{let a=t.dataset.body;history.pushState({},"",r("/")),v().then(()=>{let s=l.querySelector("textarea[name=body]");s&&(s.value=a||"",s.dispatchEvent(new Event("input")),s.focus())})})),document.querySelectorAll("[data-action=del-draft]").forEach(t=>t.addEventListener("click",async()=>{await d(`/api/misc/drafts/${t.dataset.id}`,{method:"DELETE"}),t.closest(".card").remove()}))}async function pt(){if(!m)return f();document.title="Settings \xB7 Dropicgram",l.innerHTML=`
    ...
    <div class="card">
      ...
      <div class="card userrow">
        <a href="${r(`/${i(n.username)}`)}" data-nav="${r(`/${i(n.username)}`)}">${h(n,40)} <b>${i(n.display_name)}</b> <span class="username">@${i(n.username)}</span></a>
        <button class="btn secondary" data-action="list-act" data-u="${i(n.username)}">${a}</button>
      </div>`).join(""):'<div class="empty">None yet.</div>',document.querySelectorAll("[data-action=list-act]").forEach(n=>n.addEventListener("click",async()=>{let u=n.dataset.u;await d(s.replace("{u}",u),{method:"DELETE"}),n.closest(".userrow").remove(),p("Done")}))}async function ft(){if(!m)return f();document.title="Muted words \xB7 Dropicgram",l.innerHTML=`
    ...
    </div>`;let o=l.querySelector(".sv-img"),n=()=>{o.src=E(a[s].image),document.title=`${a[s].username} \xB7 story`};l.querySelector('[data-action="sv-close"]').addEventListener("click",()=>v()),l.querySelector('[data-action="sv-prev"]').addEventListener("click",()=>{s>0&&(s--,n())}),l.querySelector('[data-action="sv-next"]').addEventListener("click",()=>{s<a.length-1?(s++,n()):v()}),n()}function bt(){document.querySelectorAll("[data-action=view-story]").forEach(e=>e.addEventListener("click",()=>{let t=Number(e.dataset.uid);l.innerHTML='<div class="skeleton"></div>',yt(t)})),document.querySelectorAll("[data-action=add-story]").forEach(e=>e.addEventListener("click",()=>{let t=document.createElement("input");t.type="file",t.accept="image/*",t.onchange=async()=>{let a=t.files?.[0];if(!a)return;let s=new FormData;s.append("image",a);try{await A("/api/stories",s),p("Story posted for 24h"),v()}catch(o){p(o.message)}},t.click()}))}function P(e){e.querySelectorAll("[data-action=like]").forEach(t=>t.addEventListener("click",async()=>{if(!m)return location.href=r("/login");let s=t.closest(".post").dataset.postid,o=t.classList.contains("liked")?"unlike":"like";t.classList.toggle("liked",o==="like");let n=t.querySelector("span");n&&(n.textContent=String(Math.max(0,Number(n.textContent)+(o==="like"?1:-1))));let u=await d(`/api/posts/${s}/${o}`,{method:"POST"});n&&(n.textContent=String(u.post.likes_count))})),e.querySelectorAll(".post").forEach(t=>{let a=0;t.addEventListener("dblclick",()=>{let s=t.querySelector("[data-action=like]");s&&!s.classList.contains("liked")&&s.click()}),t.addEventListener("touchend",s=>{let o=Date.now();if(o-a<300){let n=t.querySelector("[data-action=like]");n&&!n.classList.contains("liked")&&n.click();let u=t.querySelector(".post-img");u&&u.animate([{transform:"scale(1.15)"},{transform:"scale(1)"}],{duration:300})}a=o})}),e.querySelectorAll("[data-action=react]").forEach(t=>t.addEventListener("click",()=>{let s=t.closest(".post").dataset.postid,o=["\u2764\uFE0F","\u{1F602}","\u{1F44D}","\u{1F525}","\u{1F62E}","\u{1F622}"],n=document.createElement("div");n.className="emoji-sheet",n.innerHTML=o.map(c=>`<button class="emoji">${c}</button>`).join(""),l.appendChild(n);let u=async c=>{try{await d(`/api/posts/${s}/react`,{method:"POST",body:JSON.stringify({emoji:c})}),p(`Reacted ${c}`),v()}catch(g){p(g.message)}n.remove()};n.querySelectorAll(".emoji").forEach(c=>c.addEventListener("click",()=>u(c.textContent))),n.addEventListener("click",c=>{c.target===n&&n.remove()})})),e.querySelectorAll("[data-action=toggle-comments]").forEach(t=>t.addEventListener("click",()=>{let a=t.closest(".post").querySelector("[data-comments]");a.hidden=!a.hidden})),e.querySelectorAll("form[data-action=comment]").forEach(t=>t.addEventListener("submit",async a=>{a.preventDefault();let s=t.elements.namedItem("body"),o=t.elements.namedItem("image"),n=o?.files?.[0];if(!s.value.trim()&&!n)return;let u=t.closest(".post"),c=u.dataset.postid,g;if(n){let y=new FormData(t);g=await A(`/api/posts/${c}/comments`,y)}else g=await d(`/api/posts/${c}/comments`,{method:"POST",body:JSON.stringify({body:s.value})});D(u,g.post),s.value="",o&&(o.value=""),S()})),e.querySelectorAll("[data-action=reply]").forEach(t=>t.addEventListener("click",()=>{let a=t.closest(".post"),s=t.dataset.cid,o=a.querySelector(".reply-box");if(o)return o.remove();let n=document.createElement("div");n.className="reply-box",n.innerHTML=`<form style="display:flex;gap:8px;margin-top:8px">
        <label class="btn secondary" style="cursor:pointer;padding:8px 12px">\u{1F4F7}<input type="file" name="image" accept="image/*" hidden /></label>
        <input class="input" placeholder="Reply..." name="body"/><button class="btn">Reply</button>
      </form>`,t.closest(".comment").appendChild(n),n.querySelector("form").addEventListener("submit",async u=>{u.preventDefault();let c=n.querySelector("input[name=body]"),y=n.querySelector("input[name=image]")?.files?.[0],_=c.value.trim();if(!_&&!y)return;let b=a.dataset.postid,$;if(y){let C=new FormData(n.querySelector("form"));C.append("parent_id",String(s)),$=await A(`/api/posts/${b}/comments`,C)}else $=await d(`/api/posts/${b}/comments`,{method:"POST",body:JSON.stringify({body:_,parent_id:s})});D(a,$.post),n.remove()})})),e.querySelectorAll("[data-action=like-comment]").forEach(t=>t.addEventListener("click",async()=>{if(!m)return location.href=r("/login");let a=t.dataset.cid,s=t.closest(".post"),o=s.dataset.postid;await d(`/api/posts/comments/${a}/like`,{method:"POST"});let n=await d(`/api/posts/${o}`);D(s,n.post)})),e.querySelectorAll("[data-action=vote]").forEach(t=>t.addEventListener("click",async()=>{if(!m)return location.href=r("/login");let a=t.closest(".post"),s=a.dataset.postid,o=t.dataset.opt;try{await d(`/api/posts/polls/${a.querySelector(".poll").dataset.pid}/vote`,{method:"POST",body:JSON.stringify({option_id:o})});let n=await d(`/api/posts/${s}`);D(a,n.post)}catch(n){p(n.message)}})),e.querySelectorAll("[data-action=menu]").forEach(t=>t.addEventListener("click",a=>{a.stopPropagation();let s=t.closest(".dropdown").querySelector(".menu");s.hidden=!s.hidden})),document.addEventListener("click",()=>{document.querySelectorAll(".menu").forEach(t=>t.hidden=!0)}),e.querySelectorAll("[data-action=save],[data-action=unsave]").forEach(t=>t.addEventListener("click",async()=>{let s=t.closest(".post").dataset.postid,o=t.dataset.action==="save";await d(`/api/misc/bookmarks/${s}`,{method:o?"POST":"DELETE"}),p(o?"Saved":"Removed"),t.textContent=o?"\u{1F516} Unsave":"\u{1F516} Save",t.dataset.action=o?"unsave":"save"})),e.querySelectorAll("[data-action=pin]").forEach(t=>t.addEventListener("click",async()=>{let a=t.closest(".post");await d(`/api/posts/${a.dataset.postid}/pin`,{method:"POST"}),p("Pinned"),v()})),e.querySelectorAll("[data-action=edit]").forEach(t=>t.addEventListener("click",()=>{let a=t.closest(".post"),s=a.querySelector(".post-body"),o=s&&s.textContent||"",n=document.createElement("div");n.className="reply-box",n.innerHTML=`<form style="display:flex;gap:8px;margin-top:8px"><input class="input" value="${i(o)}"/><button class="btn">Save</button></form>`,a.querySelector(".post-actions").appendChild(n),n.querySelector("form").addEventListener("submit",async u=>{u.preventDefault();let c=n.querySelector("input").value.trim();c&&(await d(`/api/posts/${a.dataset.postid}`,{method:"PUT",body:JSON.stringify({body:c})}),n.remove(),v())})})),e.querySelectorAll("[data-action=delete]").forEach(t=>t.addEventListener("click",async()=>{let a=t.closest(".post");confirm("Delete this post?")&&(await d(`/api/posts/${a.dataset.postid}`,{method:"DELETE"}),a.remove(),p("Deleted"))})),e.querySelectorAll("[data-action=repost]").forEach(t=>t.addEventListener("click",async()=>{if(!m)return location.href=r("/login");let a=t.closest(".post");await d(`/api/posts/${a.dataset.postid}/repost`,{method:"POST"}),p("Reposted"),v()})),e.querySelectorAll("[data-action=quote]").forEach(t=>t.addEventListener("click",()=>{let a=t.closest(".post"),s=a.querySelector(".post-body"),o=s&&s.textContent||"",n=document.createElement("div");n.className="reply-box",n.innerHTML=`<form style="display:flex;gap:8px;margin-top:8px"><input class="input" placeholder="Quote..." value="${i(o)}"/><button class="btn">Quote</button></form>`,a.querySelector(".post-actions").appendChild(n),n.querySelector("form").addEventListener("submit",async u=>{u.preventDefault();let c=n.querySelector("input").value.trim();c&&(await d(`/api/posts/${a.dataset.postid}/quote`,{method:"POST",body:JSON.stringify({body:c})}),n.remove(),v())})})),e.querySelectorAll("[data-action=share]").forEach(t=>t.addEventListener("click",()=>{let a=t.closest(".post").dataset.postid,s=location.origin+r(`/post/${a}`),o=()=>{navigator.clipboard?.writeText(s).then(()=>p("Link copied")).catch(()=>p(s))};navigator.share?navigator.share({url:s}).catch(o):o()})),e.querySelectorAll("[data-action=report]").forEach(t=>t.addEventListener("click",async()=>{let a=t.closest(".post").dataset.postid,s=prompt("Why are you reporting this post?");s!==null&&(await d("/api/misc/reports",{method:"POST",body:JSON.stringify({target_type:"post",target_id:a,reason:s})}),p("Reported. Thanks."))})),e.querySelectorAll("[data-action=mute]").forEach(t=>t.addEventListener("click",async()=>{let a=t.closest(".post"),s=a.dataset.postid?(a.querySelector(".username").textContent||"").replace("@",""):"";await d(`/api/misc/mutes/${encodeURIComponent(s)}`,{method:"POST"}),p(`Muted @${s}`)})),e.querySelectorAll("[data-action=block]").forEach(t=>t.addEventListener("click",async()=>{let a=t.closest(".post"),s=(a.querySelector(".username").textContent||"").replace("@","");await d(`/api/misc/blocks/${encodeURIComponent(s)}`,{method:"POST"}),p(`Blocked @${s}`),a.remove()})),e.querySelectorAll("[data-lightbox]").forEach(t=>t.addEventListener("click",()=>{let a=document.createElement("div");a.className="lightbox",a.innerHTML=`<img src="${i(t.src)}" /><button class="btn" style="position:absolute;top:16px;right:16px">\u2715</button>`,a.addEventListener("click",s=>{(s.target===a||s.target.tagName==="BUTTON")&&a.remove()}),document.body.appendChild(a)}))}function D(e,t){let a=e.querySelector("[data-comment-list]");a&&(a.innerHTML=t.comments.map(u=>W(u,t.id)).join(""));let s=e.querySelector("[data-action=toggle-comments] span");s&&(s.textContent=String(t.comments_count));let o=e.querySelector("[data-action=like] span");o&&(o.textContent=String(t.likes_count));let n=e.querySelector("[data-action=like]");n&&n.classList.toggle("liked",t.liked_by_me)}async function v(){Q();let e=location.pathname;location.hash&&location.hash.startsWith("#post")&&(e=`/post/${location.hash.slice(5)}`),L&&e.startsWith(L)&&(e=e.slice(L.length)||"/"),e=e.replace(/^\/#.*/,"");let t=e.split("/").filter(Boolean);if(e==="/")return et();if(e==="/explore")return at();if(t[0]==="search")return st(decodeURIComponent(t[1]||""));if(t[0]==="hashtag")return nt(t[1]||"");if(t[0]==="notifications")return it();if(t[0]==="messages")return t[1]?ct(t[1]):dt();if(t[0]==="bookmarks")return lt();if(t[0]==="requests")return mt();if(t[0]==="drafts")return ut();if(t[0]==="settings"&&t[1]==="edit")return gt();if(t[0]==="settings"&&t[1]==="password")return vt();if(t[0]==="settings")return pt();if(t[0]==="blocks")return J("/api/misc/blocks","Blocked users","Unblock","/api/misc/blocks/{u}");if(t[0]==="mutes")return J("/api/misc/mutes","Muted users","Unmute","/api/misc/mutes/{u}");if(t[0]==="muted-words")return ft();if(t[0]==="stats")return ht();if(t[0]==="followers")return F(t[1]||"","followers");if(t[0]==="following")return F(t[1]||"","following");if(t[0]==="post")return rt(Number(t[1]));if(e==="/login")return f();if(t[0]==="me")return m?j(m.username):f();if(t.length===1)return j(t[0])}function $t(){U.addEventListener("click",async t=>{t.target.closest("[data-action=logout]")&&(await d("/api/auth/logout",{method:"POST"}),localStorage.removeItem("token"),m=null,k(),location.href=r("/"))}),document.addEventListener("click",t=>{let a=t.target.closest("[data-nav]");if(!a)return;t.preventDefault();let s=a.dataset.nav;location.pathname!==s&&(history.pushState({},"",s),v())}),window.addEventListener("popstate",v);let e=document.getElementById("topsearch");e&&e.addEventListener("keydown",t=>{if(t.key==="Enter"){let a=e.value.trim();a&&(history.pushState({},"",r(`/search/${encodeURIComponent(a)}`)),v())}})}function wt(){let e=localStorage.getItem("token")||"",t=location.protocol==="https:"?"wss":"ws",a=T?new URL(T).host:location.host;return`${t}://${a}/ws?token=${encodeURIComponent(e)}`}function R(){let e=localStorage.getItem("token");if(e){N=e;try{I=new WebSocket(wt())}catch{return}I.onmessage=t=>{try{let a=JSON.parse(t.data);if(a.type==="connected")return;B&&B(a)}catch{}},I.onclose=()=>{localStorage.getItem("token")===N&&setTimeout(R,3e3)}}}function Et(e){I&&I.readyState===WebSocket.OPEN&&I.send(JSON.stringify(e))}function Lt(e){if(e.type==="notification")S(),p(`\u{1F514} ${e.notification?.actor?.username||"Someone"} ${e.notification?.label||"interacted"}`);else if(e.type==="dm"){S();let t=String(e.sender_username||""),a=String(e.body||"[photo]");p(`\u{1F4E9} ${t}: ${a}`)}else if(e.type==="new_post")S();else if(e.type==="typing"){let t=Number(e.from);if(location.pathname.includes("/messages/")){let a=document.getElementById("typing-ind");a&&(a.textContent="typing\u2026")}}}async function kt(){$t(),B=Lt;try{let e=sessionStorage.getItem("redirect");if(e){sessionStorage.removeItem("redirect");try{let a=new URL(e).pathname.replace(/\/$/,"")||"/";L&&a.startsWith(L)&&(a=a.slice(L.length)||"/"),history.replaceState({},"",L+a)}catch{}}}catch{}try{m=(await d("/api/auth/me")).user,R();try{w=(await d("/api/account/settings")).settings}catch{}document.body.classList.toggle("light",w.dark_mode===0)}catch{m=null}k(),S(),v()}kt();})();
